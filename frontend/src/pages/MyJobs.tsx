...
import {
  getCachedFleetSnapshot,
  getCachedJobsSnapshot,
  getJobs,
  getFleet,
} from "@/lib/api";
...
export default function MyJobs() {
  ...
  const [jobView, setJobView] = useState<
    "all" | "condition" | "dueSoonest" | "newest"
  >("all");
  ...
  const visibleJobs: Job[] =
    jobsData.length > 0 ? jobsData : cachedJobs;

  const getJobCondition = (job: Job) => {
    const matchingBus =
      fleet.find((bus) => bus.id === job.busId) ||
      cachedFleet.find((bus) => bus.id === job.busId);

    return matchingBus?.status || "Requires Attention";
  };
  ...
  const sortedJobs = [...visibleJobs].sort((a, b) => {
    if (jobView === "condition") {
      const leftCondition = getJobCondition(a);
      const rightCondition = getJobCondition(b);

      return (
        (conditionRank[rightCondition] || 0) -
        (conditionRank[leftCondition] || 0)
      );
    }
        ...
        <div className="flex items-center justify-between">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => setJobView("condition")}>
                Condition
              </DropdownMenuItem>
                ...
                >
                  <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs font-medium ${getConditionBadgeClass(
                          condition
                        )}`}
                      >
                        {condition}
                    ...
                    <p className="text-sm text-muted-foreground">
                      ...
                      <span className="font-medium text-foreground">
                        {daysUntilDue < 0
                          ...
                          ? `${new Date(
                              job.dueDate
                            ).toLocaleDateString()} (due today)`
                          : `${new Date(
                              job.dueDate
                            ).toLocaleDateString()} (in ${daysUntilDue}d)`}
                      </span>
                    </p>
                  </div>
                  ...
                  <button
                    onClick={() => {
                      const cachedBus =
                        cachedFleet.find(
                          (bus) => bus.id === job.busId
                        ) || null;

                      if (!cachedBus && !window.navigator.onLine) {
                        toast.error(
                          "Bus details are not available offline for this job yet"
                        );
