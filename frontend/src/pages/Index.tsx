...
const Index = () => {
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <p className="text-lg font-bold text-status-urgent">
          Error loading fleet
        </p>
      </div>
    );
  ...
  const requiresAttention = fleet.filter(
    (b) => b.status === "Requires Attention"
  ).length;

  const outOfOperation = fleet.filter(
    (b) => b.status === "Out Of Operation"
  ).length;

  const openReports = fleet.filter(
    (b) => b.issueIndicator.activeCount > 0
  ).length;
  ...
  const depots = Array.from(
    fleet
      .reduce((groups, bus) => {
        ...
        return groups;
      }, new Map<string, { depotId: string; depotName: string; buses: typeof fleet }>())
      .values()
  ...
  const stats = [
    ...
    },
    {
      ...
      value: good,
      icon: CheckCircle2,
      color: "text-status-operational bg-status-operational/10",
    },
    {
      label: "Requires Attention",
      value: requiresAttention,
      icon: AlertTriangle,
      color: "text-status-service bg-status-service/10",
    },
    {
      label: "Out Of Operation",
      value: outOfOperation,
      icon: Wrench,
      color: "text-status-urgent bg-status-urgent/10",
    },
    {
      label: "With Reports",
      value: openReports,
